...
def paste_on_axes():
    ...
    import pyperclip
    global connect_dot_flag_pos,decision_dot_flag_pos,startdot_flag_pos,stoke_arrow_flag_pos
            ...
            for j in range(len(temp)):
                if(temp_count==count1):
                    ...
                    for k1 in range(4):
                        base_array.append(guideline_axes[l].lines[k1])
                    temp_kern_value,temp_kern_list,temp_delete,temp_recent,temp_cursor_data,temp_cursor_pos,letters_already_written2,temp_guideline_axes2=add_digit_in_axes(list_of_digit=temp[j],spec_axes=guideline_axes[l],baselines_objects_array=base_array)
                    set_guideLine_variables(kern_value_array1=temp_kern_value,delete_list1=temp_delete,kern_list1=temp_kern_list,recent1=SW_global.recent_input_list,lines1=temp_guideline_axes2,cursor_pos1=temp_cursor_pos,cursor_data1=temp_cursor_data,letters_already_written=letters_already_written2)
                elif(temp_count>count1):
                    save_data_to_axes_dict(kern_value1=kern_value_array,delete_list1=delete_list,recent_input_list1=SW_global.recent_input_list,cursor_pos1=SW_global.cursor_pos,cursor_data1=SW_global.cursor_data,axes_key_index=None,letters_already_written1=SW_global.letters_already_written,axesdata=guideline_axes[l],lines1=guideline_axes[l].lines,kern_list1=SW_global.kern_list)
                    newCreateGuideLine(1,None,None,None,None)
                    for k1 in range(4):
    ...
    if(SW_global.stokearrow_on_off==1):
        ...
        composite_dot()
    SW_global.global_figure.canvas.draw()
...
def onrelease(event):
    ...
    #### check and updating if need for page change
    # guideline_axes[l].set_visible(True)
    if(guideline_axes[l].get_figure()!=(event.inaxes).get_figure()):
        multipage_data_controller(axesdata=guideline_axes[l])
        select_which_page_to_redirect(axes_data=event.inaxes)
 ...
 #   global pos2i
    ...
    if(SW_global.click_x==SW_global.release_x):
        ...
        if(checkFlag==0):
            ...
            for i in range(len(guideline_axes[l].lines)-1):
                if i>4:
                    (guideline_axes[l].lines[i]).set_color("black")
            if len(SW_global.axes_data)>0:
                for i in range(len(SW_global.axes_data)):
                    for k1 in range(len(SW_global.axes_data[str(i)]["lines"])):
                        if k1>4:
                            ((SW_global.axes_data[str(i)]["lines"])[k1]).set_color("black")
            global startdot_already_applied_array
            if len(startdot_already_applied_array)>0:
                cntr=0
                for i in range(len(startdot_already_applied_array)-1):
                    if cntr==0:
                        x12=startdot_already_applied_array[i]
                        y12=startdot_already_applied_array[i+1]
                        for k in range(x12,y12):
                            if k<=len(guideline_axes[l].lines)-1:
                                guideline_axes[l].lines[k].set_color(SW_global.startdot_color)
                        cntr=cntr+1
                    else:
                        cntr=cntr-1
                ...
                if len(SW_global.axes_data)>0:
                    cntr=0
                    for j in range(len(SW_global.axes_data)):
                        for k1 in range(len(SW_global.axes_data[str(j)]["decisiondot_already_applied_array"])-1):
                            if cntr==0:
                                x12=(SW_global.axes_data[str(j)]["decisiondot_already_applied_array"])[k1]
                                y12=(SW_global.axes_data[str(j)]["decisiondot_already_applied_array"])[k1+1]
                                for k2 in range(x12,y12):
                                    if len(SW_global.axes_data[str(j)]["lines"])>=k2:
                                        ((SW_global.axes_data[str(j)]["lines"])[k2]).set_color(SW_global.startdot_color)
                                cntr=cntr+1
                            else:
                                cntr=cntr-1
            ##### decision dot
            if len(decisiondot_already_applied_array)>0:
                cntr=0
                for j in range(len(decisiondot_already_applied_array)-1):
                    if cntr==0:
                        x12=decisiondot_already_applied_array[j]
                        y12=decisiondot_already_applied_array[j+1]
                        for k1 in range(x12,y12):
                            if(k1<=len(decisiondot_already_applied_array)-1):
                                (guideline_axes[l].lines[k1]).set_color(SW_global.decisiondot_color)
                        cntr=cntr+1
                    else:
                        cntr=cntr+1
                if len(SW_global.axes_data)>0:
                    for j in range(len(SW_global.axes_data)):
                        ...
                        for k1 in range(len(SW_global.axes_data[str(j)]["decisiondot_already_applied_array"])-1):
                            if cntr==0:
                                ...
                                y12=(SW_global.axes_data[str(j)]["decisiondot_already_applied_array"])[k1+1]
                                for k2 in range(x12,y12):
                                    ((SW_global.axes_data[str(j)]["lines"])[k2]).set_color(SW_global.decisiondot_color)
                                cntr=cntr+1
                            else:
                                cntr=cntr-1
...
def save_file():
    ...
    try:
        ...
        if SW_global.back_axes!=None:
            SW_global.back_axes.set_visible(False)
        SW_global.global_figure.savefig(default_path)
        SW_global.back_axes.set_visible(True)
        return default_path
...
def NewWindow(master):
    global all_windows,guideline_axes
    guideline_axes[l].set_visible(True)
    SW_global.global_figure.canvas.draw()

    fig,canvas=multipage_create(writingareaframe=writingareaframe)
    print("This is figure",fig)
    # main_add_controller("A")
    SW_global.current_axes=SW_global.g_val
    guideline_axes[l]=SW_global.g_val
    SW_global.global_figure=fig
    SW_global.global_canvas=canvas
    fig.canvas.draw()
    # newCreateGuideLine(1,None,None,None,None)
    fig.canvas.draw()
...
def erase_dot_of_letters(d1,d2):
    for i in range(len(SW_global.axes_data)):
        lines1=SW_global.axes_data[str(i)]["lines"].copy()
        delete1=SW_global.axes_data[str(i)]["delete_list"].copy()
        kern_list1=SW_global.axes_data[str(i)]["kern_list"].copy()
        kern_value_array1=SW_global.axes_data[str(i)]["kern_value_array"].copy()
        letters_already_written1=SW_global.axes_data[str(i)]["letters_already_written"].copy()
        recent_input_list1=SW_global.axes_data[str(i)]["recent_input_list"].copy()
        while len(delete1)>0:
            len1=len(letters_already_written1)
            len2=len1-1
            srt_loop=letters_already_written1[len2-1]
            end_loop=letters_already_written1[len2]
            for i in range(srt_loop,end_loop):
                lines1[i].set_visible(False)
            
            if len(letters_already_written1)!=0:
                del letters_already_written1[len1-1]
                del letters_already_written1[len1-2]
                last_input_len=len(delete_list1)
                last_glyph=delete_list[last_input_len-1]
            del delete1[last_input_len-1]
            l12=len(kern_value_array1)
            del kern_value_array1[l12-1]
        SW_global.global_figure.canvas.draw()
        draw_dot_of_letters_of_multiple_guideline(d1,d2)


    return

def draw_dot_of_letters_of_multiple_guideline(d1,d2):
    for j in range(len(SW_global.axes_data)):
        base_array=[]
        base_array.extend(SW_global.axes_data[str(j)]["lines"])
        temp_kern_value1,temp_kern_list1,temp_delete1,temp_recent1,temp_cursor_data1,temp_cursor_pos1,letters_already_written3,temp_guideline_axes1=add_digit_in_axes(list_of_digit=SW_global.axes_data[str(j)]["delete_list"],spec_axes=SW_global.axes_data[str(j)]["axis_data"],baselines_objects_array=base_array)
        SW_global.axes_data[str(j)]["letters_already_written"]=letters_already_written3.copy()
        SW_global.axes_data[str(j)]["lines"]=temp_guideline_axes1.copy()
    return
...
def ShowChoice():
    global stoke_arrows,index_of_composit_dot
    index_of_compositdot=v.get()
    styling_of_compsitdot(style_index_no=v.get())
    stoke_arrows.destroy()
...
stoke_arrows=None
index_of_composit_dot=0

def styling_of_compsitdot(style_index_no=None):
    global compositedot_already_applied_array
    if style_index_no!=None:
        if len(compositedot_already_applied_array)>0:
            if int(style_index_no)==0:
                ok1=1

            elif int(style_index_no)==1:
                if len(SW_global.axes_data)>0:
                    for j in range(len(SW_global.axes_data)):
                        count=-1
                        start_index=2
                        cntr=0
                        end_index=len(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])-1
                        for k1 in range(len(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])-1):
                            if cntr==0:
                                if k1>=start_index:
                                    x12=(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])[k1]
                                    y12=(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])[k1+1]
                                    for k2 in range(x12,y12):
                                        ((SW_global.axes_data[str(j)]["lines"])[k2]).set_visible(False)
                                cntr=cntr+1
                            else:
                                cntr=cntr-1
                ...
                start_index=2
                cntr=0
                end_index=len(compositedot_already_applied_array)-1 
                for k1 in range(len(compositedot_already_applied_array)-1):
                    if cntr==0:
                        if k1>=start_index:
                            x12=compositedot_already_applied_array[k1]
                            y12=compositedot_already_applied_array[k1+1]
                            for k2 in range(x12,y12):
                                (guideline_axes[l].lines[k2]).set_visible(False)
                        cntr=cntr+1
                    else:
            ...
            elif int(style_index_no)==2:
                ###### need to add function for first letter of every word is arrowed
                if len(compositedot_already_applied_array)>0:
                    if len(SW_global.axes_data)>0:
                        for j in range(len(SW_global.axes_data)):
                            word_space_index=[]
                            count=-1
                            for k1 in range(len(SW_global.axes_data[str(j)]["delete_list"])):
                                if ((SW_global.axes_data[str(j)]["delete_list"])[k1])==" ":
                                    word_space_count.append(k1)
                            if len(Word_space_index)==0:
                                if (len(SW_global.axes_data[str(j)]["delete_list"])>0) and ((SW_global.axes_data[str(j)]["delete_list"])[0]!=" "):
                                    start_index=2
                                    end_index=len(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])-1
                                    cntr=0
                                    for k2 in range(len(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])-1):
                                        if cntr==0:
                                            if k2>=start_index:
                                                x12=(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])[k2]
                                                y12=(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])[k2+1]
                                                for k3 in range(x12,y12):
                                                    ((SW_global.axes_data[str(j)]["lines"])[k3]).set_visible(False)
                                            cntr=cntr+1
                                        else:
                                            cntr=cntr-1
                            elif len(word_space_index)>0:
                                for k2 in range(len(word_space_index)-1):
                                    if word_space_index[k2]+1!=word_space_index[k2+1]:
                                        start_index=word_space_index[k2]+1+1
                                        end_index=word_space_index[k2+1]
                                        if (start_index<end_index) and (end_index<=len(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])):
                                            start=(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])[start_index*2]
                                            end=(SW_global.axes_data[str(j)][["compositedot_already_applied_array"]])[end_index*2+1]
                                            cntr=0
                                            for k5 in range(len(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])-1):
                                                if cntr==0:
                                                    # print()
                                                    if ((k5>=start) and (k5<=end)):
                                                        x12=(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])[k5]
                                                        y12=(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])[k5+1]
                                                        for k6 in range(x12,y12):
                                                            ((SW_global.axes_data[str(j)]["lines"])[k6]).set_visible(False)
                                                    cntr=cntr+1
                                                else:
                                                    cntr=cntr-1 
                        word_index_count=[]
                        for k1 in range(len(delete_list)):
                            if delete_list[k1]==" ":
                                word_index_count.append(k1)
                        if len(word_index_count)==0:
                            # print()
                            start_index=2
                            end_index=len(compositedot_already_applied_array)-1
                            cntr=0
                            for k2 in range(len(compositedot_already_applied_array)-1):
                                if (cntr==0) and(k2>=start_index):
                                    x12=compositedot_already_applied_array[k2]
                                    y12=compositedot_already_applied_array[k2+1]
                                    for k3 in range(x12,y12):
                                        (guideline_axes[l].lines[k3]).set_visible(False)
                        elif len(word_index_count)>0:
                            for k2 in range(len(word_space_index)-1):
                                if word_index_count[k2]+1!=word_index_count[k2+1]:
                                    start_index=word_space_index[k2]+1+1
                                    end_index=word_space_index[k2]-1
                                    if (start_index<end_index) and (end_index<=len(SW_global.axes_data[str(j)]["compositedot_already_applied_array"])-1):
                                        start=start_index*2
                                        end=(end_index*2)+1
                                        cntr=0
                                        for k3 in range(len(compositedot_already_applied_array)-1):
                                            if (cntr==0):
                                                if k3>=start_index and k3<=end_index:
                                                    x12=compositedot_already_applied_array[k3]
                                                    y12=compositedot_already_applied_array[k3+1]
                                                    for k4 in range(x12,y12):
                                                        (guideline_axes[l].lines[k4]).set_visible(False)
                                                cntr=cntr+1
                                            else:
                                                cntr=cntr-1
                                    
            elif int(style_index_no)==3:
                ok1=0
            else:
                ok2=0
    SW_global.global_figure.canvas.draw()
...
def eraseCompositeDotFromMultipleGuideLine():
    ...
    for i in range(len(SW_global.axes_data)):
        cntr=0
        len1=len(SW_global.axes_data[str(i)]["compositedot_already_applied_array"])-1
        for j in range(len1):
            if cntr==0:
                x12=(SW_global.axes_data[str(i)]["compositedot_already_applied_array"])[j]
                y12=(SW_global.axes_data[str(i)]["compositedot_already_applied_array"])[j+1]
                for k1 in range(x12,y12):
                    ((SW_global.axes_data[str(i)]["lines"])[k1]).set_visible(False)
                cntr=cntr+1
            else:
                cntr=cntr-1
        SW_global.axes_data[str(i)]["stoke_arrow_flag_pos"]=0
...
def composite_dot():
    global stoke_arrow_flag_pos,fig
    ...
    if SW_global.startdot_on_off == 1:
        ...
        if startdot_flag_pos1 == 0:
            for i in range(delete_list_counter):
                ...
                if glyph==" ":
                    c1,c2=[0],[0]
                    te=guideline_axes1_plot.plot(c1, c2, marker='.', color="white")
                    te[0].set_visible(False)
                    guideline_axes1_lines.append(te[0])
                else:
                    c1, c2 = manuscript_start_dot.return_startdot_manuscript(glyph)
                    c1, c2, draw_type = Kern_add_help.kern_add_operation(c1, c2, krn)
...
def eraseStartDotFromMultipleGuideLine():
    ...
    for i in range(len(SW_global.axes_data)):
        len1=len(SW_global.axes_data[str(i)]["startdot_already_applied_array"])-1
        cntr=0
        SW_global.axes_data[str(i)]["startdot_flag_pos"]=0
        for j in range(len1):
            if cntr==0:
                x12 = (SW_global.axes_data[str(i)]["startdot_already_applied_array"])[j]
                y12 = (SW_global.axes_data[str(i)]["startdot_already_applied_array"])[j+1]
                for k1 in range(x12,y12):
                    ((SW_global.axes_data[str(i)]["lines"])[k1]).set_visible(False)
                cntr=cntr+1
            else:
...
def start_dot():
    global startdot_flag_pos,guideline_axes,startdot_already_applied_array,delete_list,kern_value_array
    if SW_global.startdot_on_off == 1:
        delete_list_counter = len(delete_list)
        ...
        if startdot_flag_pos == 0:
            for i in range(delete_list_counter):
                ...
                if glyph==" ":
                    c1,c2=[0],[0]
                    guideline_axes[l].plot(c1, c2, marker='.', color="white")
...
def Decision_dot():
    global decision_dot_flag_pos,decisiondot_already_applied_array,delete_list,kern_value_array,guideline_axes,l
    # print("I am in decision dot ",decision_dot_flag_pos)
    global fig
    if SW_global.decisiondot_on_off == 1:
        delete_list_counter = len(delete_list)
        ...
        if decision_dot_flag_pos == 0:
            for i in range(delete_list_counter):
                ...
                if glyph==" ":
                    c1,c2=[0],[0]
                    guideline_axes[l].plot(c1, c2, marker='.', color="white", linewidth=0.1)
                else:
            ...
            # print("This is decision odr flag pos",decision_dot_flag_pos)
            # print("This is decision dot delete_list_counter",delete_list_counter)
    ...
    if SW_global.decisiondot_on_off == 1:
        ...
        if decision_dot_flag_pos1 == 0:
            ...
            for i in range(delete_list_counter1):
                ...
                if glyph==" ":
                    c1,c2=[0],[0]
                    te1=guideline_axes_plot.plot(c1, c2, marker='.', color="white", linewidth=0.1)
...
def decisionDotEraseMultiple():
    ...
    for i in range(len(SW_global.axes_data)):
        len1=len(SW_global.axes_data[str(i)]["decisiondot_already_applied_array"])-1
        cntr=0
        for j in range(len1):
            if cntr==0:
                x12=(SW_global.axes_data[str(i)]["decisiondot_already_applied_array"])[j]
                y12=(SW_global.axes_data[str(i)]["decisiondot_already_applied_array"])[j+1]
                for k1 in range(x12,y12):
                    ((SW_global.axes_data[str(i)]["lines"])[k1]).set_visible(False)
                cntr=cntr+1
            else:
...
def main():
    global initial_decision_dot_pos, last_decision_dot_pos, decision_dot_flag_pos,decisiondot_already_applied_array
    if SW_global.decisiondot_on_off == 0:
        ...
        for ii in range(fln - 1):
            if cntr == 0:
                ...
                for jj in range(x12, y12):
                    if len(guideline_axes[l].lines)>=jj:
                        guideline_axes[l].lines[jj].set_visible(False)
                cntr = cntr + 1
            else:
                cntr = cntr - 1
    ...
    if SW_global.connectdot_on_off == 1:
        ...
        if connect_dot_flag_pos1 == 0:
            for i in range(delete_list_counter):
                ...
                if glyph==" ":
                    c1,c2=[0],[0]
                    te=d.plot(c1, c2, color="white", linewidth=0.5)
                    guideline_axes1_lines.append(te[0])
                else:
...
def eraseConnectDotFromMultipleGuideLine():
    ...
    for i in range(len(SW_global.axes_data)):
        len1=len(SW_global.axes_data[str(i)]["connectdot_already_applied_array"])-1
        cntr=0
        for j in range(len1):
            if cntr==0:
                x12=(SW_global.axes_data[str(i)]["connectdot_already_applied_array"])[j]
                y13=(SW_global.axes_data[str(i)]["connectdot_already_applied_array"])[j+1]
                for k1 in range(x12,y13):
                    ((SW_global.axes_data[str(i)]["lines"])[k1]).set_visible(False)
                cntr=cntr+1
            else:
                cntr=cntr-1
...
def connect_dot():
    global connect_dot_flag_pos,guideline_axes,delete_list,kern_value_array,connectdot_already_applied_array
    global fig
    if SW_global.connectdot_on_off == 1:
        delete_list_counter = len(delete_list)
        ...
        if connect_dot_flag_pos == 0:
            for i in range(delete_list_counter):
                ...
                if glyph==" ":
                    c1,c2=[0],[0]
                    guideline_axes[l].plot(c1, c2, color="white", linewidth=0.5)
                else:
                    skip_list=["A","E","Y","F","M","N","X","z","Z","x","K","[","]","w","W","space"]
                    if glyph in skip_list:
                        c1, c2 = manuscript_connect_dot.return_manuscript_fonts(glyph)
...
def main4():
    global initial_connect_dot_pos, last_connect_dot_pos, connect_dot_flag_pos,connectdot_already_applied_array,fig
    ...
    if SW_global.connectdot_on_off == 0:
        ...
        for ii in range(fln - 1):
            if cntr == 0:
                ...
                for jj in range(x12, y12):
                    if(jj<=len(guideline_axes[l].lines)):
                        guideline_axes[l].lines[jj].set_visible(False)
                cntr = cntr + 1
            else:
                cntr = cntr - 1
...
def color_letters_draw_fun():
    global color_letter_flag_pos, color_letter_features_on_off, d1, d2, alp,fig
...
def color_letter_continues_write():
    ...
    try:
        ...
        print(e)
        pass
def color_letter_for_entire():
    for j in range(len(SW_global.axes_data)):
        for k1 in range(len(SW_global.axes_data[str(j)]["letters_already_written"])-1):
            for k2 in range((SW_global.axes_data[str(j)]["letters_already_written"])[k1],(SW_global.axes_data[str(j)]["letters_already_written"])[k1+1]):
                ((SW_global.axes_data[str(j)]["lines"])[k2]).set_visible(False)
        (SW_global.axes_data[str(j)]["letters_already_written"]).clear()
        (SW_global.axes_data[str(j)]["recent_input_list"]).clear()
        (SW_global.axes_data[str(j)]["kern_value_array"]).clear()
        (SW_global.axes_data[str(j)]["kern_list"]).clear()
        (SW_global.axes_data[str(j)]["kern_list"]).append(0)
        (SW_global.axes_data[str(j)]["kern_value_array"]).append(0)
        color_letter_flag_pos=0
        d=[]

        for k3 in range(len(SW_global.axes_data[str(j)]["delete_list"])):
            color_letter_flag_pos = color_letter_flag_pos + 1
            length12 = len(SW_global.axes_data[str(j)]["recent_input_list"])
            user_input = (SW_global.axes_data[str(j)]["delete_list"])[k3]
            # if user_input!=" ":
                
            
            if user_input!=" ":
                x_max = manuscript.x_max[user_input]
                kern_x = (SW_global.axes_data[str(j)]["kern_list"])[0]
                skip_list=["A","E","Y","F","M","N","X","z","Z","x","K","[","]","w","W","space"]
                if user_input in skip_list:
                    c1, c2 = manuscript_Color_Letters.return_manuscript_color_fonts(user_input)
                else:
                    ...
                    c1,c2=font_check(x,y)
                c1, c2, draw_type = Kern_add_help.kern_add_operation(c1, c2, kern_x)
                kern_x = (SW_global.axes_data[str(j)]["kern_list"])[0] + x_max + 300
                (SW_global.axes_data[str(j)]["kern_list"]).insert(0, kern_x)
                kern_counter = len(SW_global.axes_data[str(j)]["kern_value_array"])
                (SW_global.axes_data[str(j)]["kern_value_array"]).insert(kern_counter, kern_x)
                (SW_global.axes_data[str(j)]["recent_input_list"]).insert(length12, user_input)
                d.insert(length12,user_input)
                init_enrty_pos = len(SW_global.axes_data[str(j)]["letters_already_written"])
                inti_letter_pos = len(SW_global.axes_data[str(j)]["lines"])
                SW_global.axes_data[str(j)]["letters_already_written"].insert(init_enrty_pos, inti_letter_pos)
                if draw_type == 1:
                    k4=(SW_global.axes_data[str(j)]["axis_data"]).plot(c1, c2, color=SW_global.first_letter_background_color, linewidth=0.7,
                                       dashes=(d1, d2), alpha=alp)
                    (SW_global.axes_data[str(j)]["lines"]).append(k4[0])
                else:
                    n = len(c1)
                    for i in range(n):
                        if i == 0:
                            k4=(SW_global.axes_data[str(j)]["axis_data"]).plot(c1[i], c2[i], color=SW_global.first_letter_background_color, linewidth=0.7,
                                               dashes=(d1, d2), alpha=alp)
                            (SW_global.axes_data[str(j)]["lines"]).append(k4[0])
                        if i == 1:
                            k4=(SW_global.axes_data[str(j)]["axis_data"]).plot(c1[i], c2[i], color=SW_global.second_letter_background_color, linewidth=0.7,
                                               dashes=(d1, d2), alpha=alp)
                            (SW_global.axes_data[str(j)]["lines"]).append(k4[0])
                        if i == 2:
                            k4=(SW_global.axes_data[str(j)]["axis_data"]).plot(c1[i], c2[i], color=SW_global.third_letter_background_color, linewidth=0.7,
                                               dashes=(d1, d2), alpha=alp)
                            (SW_global.axes_data[str(j)]["lines"]).append(k4[0])
                        
                        if i == 3:
                            k4=(SW_global.axes_data[str(j)]["axis_data"]).plot(c1[i], c2[i], color=SW_global.forth_letter_background_color, linewidth=0.7,
                                               dashes=(d1, d2), alpha=alp)
                            (SW_global.axes_data[str(j)]["lines"]).append(k4[0])
                        

                    final_enrty_pos = len(SW_global.axes_data[str(j)]["letters_already_written"])
                    final_letter_pos = len(SW_global.axes_data[str(j)]["lines"])
                    (SW_global.axes_data[str(j)]["letters_already_written"]).insert(final_enrty_pos, final_letter_pos)
            else:
                kern_x = (SW_global.axes_data[str(j)]["kern_list"])[0]
                kern_x = (SW_global.axes_data[str(j)]["kern_list"])[0]
                c1,c2=[0],[0]
                kern_x = (SW_global.axes_data[str(j)]["kern_list"])[0] + 300
                (SW_global.axes_data[str(j)]["kern_list"]).insert(0, kern_x)
                kern_counter = len(SW_global.axes_data[str(j)]["kern_value_array"])
                (SW_global.axes_data[str(j)]["kern_value_array"]).insert(kern_counter, kern_x)
                (SW_global.axes_data[str(j)]["recent_input_list"]).insert(length12, user_input)
...
def change_color_letter_to_black_normal():
    for j in range(len(SW_global.axes_data)):
        cntr=0
        for k1 in range(len(SW_global.axes_data[str(j)]["letters_already_written"])-1):
            if cntr==0:
                x12=(SW_global.axes_data[str(j)]["letters_already_written"])[k1]
                y12=(SW_global.axes_data[str(j)]["letters_already_written"])[k1+1]
                for k1 in range(x12,y12):
                    ((SW_global.axes_data[str(j)]["lines"])[k1]).set_color('black')
                cntr=cntr+1
            else:
                cntr=cntr-1
    SW_global.global_figure.canvas.draw()
    # for j in range(len(SW_global.axes_data)):
    #     len1=len(SW_global.axes_data[str(j)]["letters_already_written"])-1
    #     for k1 in range()
    # SW_global.global_figure.canvas.draw()
    return
...
def main12345():
    ...
    if color_letter_features_on_off == 0:
        ...
        change_color_letter_to_black_normal()
        color_letter_flag_pos = 0
...
def main_add_controller(event_key=None):
    global guideline_axes,compositedot_already_applied_array,startdot_already_applied_array,decisiondot_already_applied_array,connectdot_already_applied_array,stoke_arrow_flag_pos,startdot_already_applied_array,\
        stoke_arrow_flag_pos,decision_dot_flag_pos,connect_dot_flag_pos,startdot_flag_pos
    if((event_key!=None) and(SW_global.current_axes!=None)):
        k90=(SW_global.current_pos!=None) and (len(SW_global.cursor_pos)>0) and (SW_global.current_pos<SW_global.cursor_pos[len(SW_global.cursor_pos)-1])
        # print((SW_global.current_pos!=None) and (len(SW_global.cursor_pos)>0) and (SW_global.current_pos<SW_global.cursor_pos[len(SW_global.cursor_pos)-1]))
        #### Here We have to add a block for SW_global.current_axes==guideline_axes[l] and SW_global.current_pos!=SW_global.cursor_pos[-1]
        if(SW_global.current_axes==guideline_axes[l]):
            # print("This chheck point 1")
            if((len(SW_global.kern_list)>0)and(SW_global.kern_list[0]>SW_global.max_limit) and((SW_global.current_pos==None) or((len(SW_global.cursor_pos)>0) and (SW_global.current_pos>=SW_global.cursor_pos[len(SW_global.cursor_pos)-1])))):
                ...
                if(event_key==" "):
                    ...
                    if(" " in delete_list):
                        ...
                        # print("Check point 66666 ")
                        base_array=[]
                        for k1 in range(4):
                            base_array.append((SW_global.axes_data[str(len(SW_global.axes_data)-1)]["lines"])[k1])
                        temp_kern_value,temp_kern_list,temp_delete,temp_recent,temp_cursor_data,temp_cursor_pos,letters_already_written2,temp_guideline_axes2=add_digit_in_axes(list_of_digit=SW_global.delete_list_divide[0],spec_axes=temp_axes_list[0],baselines_objects_array=base_array)
                        # print("check point")
                        ...
                        # print("check point 77777")
                        SW_global.axes_data[str(len(SW_global.axes_data)-1)]["recent_input_list"]=temp_recent.copy()
                        (SW_global.axes_data[str(len(SW_global.axes_data)-1)]["compositedot_already_applied_array"]).clear()
                        (SW_global.axes_data[str(len(SW_global.axes_data)-1)]["startdot_already_applied_array"]).clear()
                        (SW_global.axes_data[str(len(SW_global.axes_data)-1)]["decisiondot_already_applied_array"]).clear()
                        (SW_global.axes_data[str(len(SW_global.axes_data)-1)]["connectdot_already_applied_array"]).clear()
                        SW_global.axes_data[str(len(SW_global.axes_data)-1)]["stoke_arrow_flag_pos"]=0
                        SW_global.axes_data[str(len(SW_global.axes_data)-1)]["connect_dot_flag_pos"]=0
                        SW_global.axes_data[str(len(SW_global.axes_data)-1)]["decision_dot_flag_pos"]=0
                        SW_global.axes_data[str(len(SW_global.axes_data)-1)]["startdot_flag_pos"]=0
                        ...
                        base_array1=[]
                        for k1 in range(4):
                            base_array1.append(guideline_axes[l].lines[k1])
                        temp_kern_value1,temp_kern_list1,temp_delete1,temp_recent1,temp_cursor_data1,temp_cursor_pos1,letters_already_written3,temp_guideline_axes1=add_digit_in_axes(list_of_digit=SW_global.delete_list_divide[1],spec_axes=temp_axes_list[1],baselines_objects_array=base_array1)
                        kern_value_array.clear()
                        delete_list.clear()
                        ...
                        guideline_axes[l].lines=temp_guideline_axes1.copy()
                        startdot_flag_pos=0
                        decision_dot_flag_pos=0
                        stoke_arrow_flag_pos=0
                        connect_dot_flag_pos=0
                        ...
                        # print("Don't need to check j?ust use normal operation ")
                        newCreateGuideLine(1,None,None,None,None)
                        SW_global.recent_input_list.clear()
                        SW_global.kern_list.clear()
                        SW_global.kern_list.append(0)
                        ...
                        reset_features_flag()
                        add_any_letter_with_space_from_rear_side(event_key=event_key)
            elif((SW_global.current_pos!=None) and (len(SW_global.cursor_pos)>0) and (SW_global.current_pos<SW_global.cursor_pos[len(SW_global.cursor_pos)-1])):
                # print("checjk")
                ...
                if(len(temp10)>1):
                    ...
                    temp_kern_value1,temp_kern_list1,temp_delete1,temp_recent1,temp_cursor_data1,temp_cursor_pos1,letters_already_written3,temp_guideline_axes1=add_digit_in_axes(list_of_digit=temp10[0],spec_axes=guideline_axes[l],baselines_objects_array=base_array)
                    ...
                    for j in range(3):
                        base_array.append(guideline_axes[l].lines[j])
                    temp_kern_value1,temp_kern_list1,temp_delete1,temp_recent1,temp_cursor_data1,temp_cursor_pos1,letters_already_written3,temp_guideline_axes1=add_digit_in_axes(list_of_digit=temp10[1],spec_axes=guideline_axes[l],baselines_objects_array=base_array)
                    kern_value_array.clear()
                    delete_list.clear()
                    SW_global.kern_list=temp_kern_list1.copy()
                    # print("temp_delete1",temp_delete1)
                    ...
                    for j in range(len(SW_global.axes_data)):
                        if(SW_global.axes_data[str(j)]["axis_data"]==SW_global.current_axes):
                            if(SW_global.current_pos_in_number<len(SW_global.cursor_data)-1):
                                ...
                                SW_global.current_axes=guideline_axes[l]
    # print("This is end of main add controller "*1000)
    # print("This is testing point 1")
    ...
    if(SW_global.current_axes==guideline_axes[l]):
        ...
        if(event_key!=None and key_axes!=None and SW_global.current_pos_in_number!=None):
            ...
            if(SW_global.current_axes==guideline_axes[l]):
                ...
                SW_global.delete_list_divide.clear()
    # print("end of main add controller "*1000)
        ...
        if(SW_global.current_axes==guideline_axes[l]):
            if((SW_global.kern_list[0]>SW_global.max_limit) and((SW_global.current_pos==None) or(SW_global.current_pos>=SW_global.cursor_pos[len(SW_global.cursor_pos)-1]))):
                ...
                if(len(temp10)>1):
                    ...
                    clear_digit_from_axes(axesdata=guideline_axes[l])
                    base_array1=[guideline_axes[l].lines[k3] for k3 in range(4)]
                    temp_kern_value1,temp_kern_list1,temp_delete1,temp_recent1,temp_cursor_data1,temp_cursor_pos1,letters_already_written3,temp_guideline_axes1=add_digit_in_axes(list_of_digit=temp10[0],spec_axes=guideline_axes[l],baselines_objects_array=base_array1)
                    kern_value_array.clear()
                    delete_list.clear()
                    SW_global.kern_list=temp_kern_list1.copy()
                    reset_guide_line_delete_kern_value(temp_delete1,temp_kern_value1)                    
            ...
            if(SW_global.current_axes!=None and guideline_axes[l]==SW_global.current_axes):
                ...
                if(sum1>SW_global.max_limit):
                    ...
                    for j in temp12:
                        temp10.append(j)
                    base_array1=[]
                    base_array1.extend(SW_global.axes_data[str(key_axes)]["lines"])
                    temp_kern_value1,temp_kern_list1,temp_delete1,temp_recent1,temp_cursor_data1,temp_cursor_pos1,letters_already_written3,temp_guideline_axes1=add_digit_in_axes(list_of_digit=temp10[0],spec_axes=SW_global.axes_data[str(key_axes)]["axis_data"],baselines_objects_array=base_array1)
                    save_data_to_axes_dict(kern_value1=temp_kern_value1.copy(),
                        delete_list1=temp_delete1.copy(),recent_input_list1=temp_recent1.copy(),
                        cursor_pos1=temp_cursor_pos1.copy(),
                        cursor_data1=temp_cursor_data1.copy(),
                ...
                if(axesdata==guideline_axes[l]):
                    ...
                    if(len(temp11)>1):
                        if(key_for_axes_no+1>len(next_axes1)):
                            ...
                            base_array=[guideline_axes[l].lines[k1] for k1 in range(4)]
                            ...
                            newCreateGuideLine(1,None,None,None)
                            base_array=[guideline_axes[l].lines[k1] for k1 in range(4)]
                            temp_kern_value11,temp_kern_list11,temp_delete11,temp_recent11,temp_cursor_data11,temp_cursor_pos11,letters_already_written31,temp_guideline_axes11=add_digit_in_axes(list_of_digit=temp10[1].copy(),spec_axes=guideline_axes[l],baselines_objects_array=base_array)
                            kern_value_array.clear()
                            kern_value_array.extend(temp_kern_value11.copy())
                            delete_list.clear()
                            delete_list.extend(temp_delete11.copy())
                            ...
                            clear_digit_from_axes(axesdata=guideline_axes[l])
                            base_array=[guideline_axes[l].lines[k2] for k2 in range(4)]
                        ...
                        clear_digit_from_axes(axesdata=guideline_axes[l])
                        base_array=[guideline_axes[l].lines[k2] for k2 in range(4)]
                        temp_kern_value11,temp_kern_list11,temp_delete11,temp_recent11,temp_cursor_data11,temp_cursor_pos11,letters_already_written31,temp_guideline_axes11=add_digit_in_axes(list_of_digit=temp11[0].copy(),spec_axes=guideline_axes[l],baselines_objects_array=base_array)
                        #save_data_to_axes_dict(kern_value1=temp_kern_value1.copy(),delete_list1=temp_delete1.copy(),recent_input_list1=temp_recent1.copy(),cursor_pos1=temp_cursor_pos1.copy(),cursor_data1=temp_cursor_data1.copy(),kern_list1=temp_kern_list1.copy(),axes_key_index=None,letters_already_written1=letter_already_written3.copy(),lines1=temp_guideline_axes1.copy(),axesdata=guideline_axes[l])
                        kern_value_array.clear()
                        kern_value_array.extend(temp_kern_value11.copy())
                        delete_list.clear()
...
def check_sum_of_digit_advanced(array_value=None,key_axes=None):
    global guideline_axes
    # print("This is check sum of digit advanced ",guideline_axes[l].lines)
...
def reset_features_flag():
    global stoke_arrow_flag_pos,decision_dot_flag_pos,connect_dot_flag_pos,startdot_flag_pos
    stoke_arrow_flag_pos=0
    decision_dot_flag_pos=0
    connect_dot_flag_pos=0
    startdot_flag_pos=0
...
def reset_guide_line_delete_kern_value(delete_list1,temp_kern_value1):
    ...
    global delete_list,kern_value_array
...
def new_create_text_box(n,shift_no):
   ...
   #     SW_global.count_for_height=1
    ...
    if(SW_global.back_axes==None):
        SW_global.back_axes=SW_global.global_figure.add_axes([old_l, old_b-height_axes, 0.98, 0.156],frameon=True)
...
def newCreateGuideLine(n,a,b,c,d):
   ...
   #     SW_global.count_for_height=1
    ...
    #     pass
    if(SW_global.back_axes==None):
        SW_global.back_axes=SW_global.global_figure.add_axes([old_l, old_b, 0.98, 0.156],frameon=True)
        SW_global.back_axes.set_xticks([])
        SW_global.back_axes.set_yticks([])
        SW_global.back_axes.spines["bottom"].set_linestyle('dashed')
        SW_global.back_axes.spines["bottom"].set_capstyle("butt")
        ...
        # print("*"*200)
        pos22=SW_global.back_axes.get_position()
        if guideline_axes[l]!=None:
            SW_global.back_axes.set_position([pos22.x0, pos22.y0-guideline_axes[l].get_position().height-0.03, 0.98, ((guideline_axes[l].get_position().height)+0.028)*SW_global.count_for_height])
        else:
            SW_global.back_axes.set_position([old_l, old_b-height_axes, 0.98, 0.156*SW_global.count_for_height])
    pos11=None
    ...
    import random
    ## This was for the creating guideline in new we was creating without font #####
    if SW_global.current_font_size==None:
        # print("check point beta ***"*10)
        if(pos11!=None):
           # print("This is width",pos11.height)
            guideline_axes[l]=SW_global.global_figure.add_axes([pos11.x0, pos11.y0-pos11.height-(random.random()/100000)-0.04, 0.96, 0.15])#SW_global.global_figure.add_axes([pos11.x0, pos11.y0-pos11.height-(random.random()/100000)-0.04, 0.96, 0.15])#plt.axes([pos11.x0, pos11.y0-pos11.height-(random.random()/100000)-0.04, 0.96, 0.15])
        else:
            guideline_axes[l]=SW_global.global_figure.add_axes([old_l, old_b-height_axes, 0.96, 0.15])
    else:
        # print("check point alpha *****"*10)
       # print("This is width",pos11.height)
        # print("This is SW_global.current_width",SW_global.current_width)
        # print("This is pos 11",pos11)
        if(SW_global.current_width!=None): 
            if(pos11!=None):
                # print("I am on  set variabe")
                guideline_axes[l]=SW_global.global_figure.add_axes([pos11.x0, pos11.y0-pos11.height-(random.random()/100000),0.96 , SW_global.current_width])
            else:
                guideline_axes[l]=SW_global.global_figure.add_axes([old_l, old_b-height_axes, 0.96, SW_global.current_width])

    # if(pos11!=None):
    #     guideline_axes[l]=plt.axes([pos11.x0, pos11.y0-0.15-(random.random()/100000), 0.96, 0.23])
    # else:
    #     guideline_axes[l] = plt.axes([old_l, old_b-height_axes, 0.96, 0.23])
    guideline_axes[l].set_xticks([])
    guideline_axes[l].set_yticks([])
    # guideline_axes[l].set_visible(False)
    SW_global.g_val=guideline_axes[l]
   ...
   # guideline_axes[l].add_patch(patches.Rectangle((776820, 5000),3000,3500,fill=False,zorder=2,closed=True,alpha=alp))
    ...
    ### for size changing#####
    SW_global.top = sl_t
    SW_global.bottom = sl_b-height_axes
    mainselector.extents = (SW_global.left, SW_global.right, SW_global.bottom, SW_global.top)
    fig.canvas.draw()
    SW_global.global_figure.canvas.draw()
    return
...
def reset_main_selector():
    ...
    #mainselector.update()
    return
def save_data_for_entire_one_page(fig=None):
    # print("I am in save data for entire one page")
    global guideline_axes,kern_value_array,delete_list,color_letter_features_on_off
    a=dict()
    if(fig!=None):
        a["fig"]=SW_global.global_figure
        a["canvas"]=canvas
    # print("This is fig",fig)
    a["axes_data"]=SW_global.axes_data.copy()
    a["box_data"]=SW_global.box_data.copy()
    a["guideline_axes"]=guideline_axes[l]
    a["delete_list"]=delete_list.copy()
    a["guideline_axes_lines"]=(guideline_axes[l].lines).copy()
    a["kern_value_array"]=kern_value_array.copy()
    ...
    a["stoke_arrow_flag_pos"]=stoke_arrow_flag_pos
    a["startdot_flag_pos"]=startdot_flag_pos
    a["SW_global.kern_list"]=SW_global.kern_list.copy()
    a["SW_global.recent_input_list"]=SW_global.recent_input_list.copy()
    a["SW_global.letters_already_written"]=SW_global.letters_already_written.copy()
    a["l"]=l
    ### Button typing status #### 
    a["SW_global.startdot_on_off"]=SW_global.startdot_on_off
    a["SW_global.decisiondot_on_off"]=SW_global.decisiondot_on_off
    a["SW_global.stokearrow_on_off"]=SW_global.stokearrow_on_off
    a["SW_global.connectdot_on_off"]=SW_global.connectdot_on_off
    a["letter_out_line_on_off"]=letter_out_line_on_off
    a["color_letter_features_on_off"]=color_letter_features_on_off
    a["SW_global.back_axes"]=SW_global.back_axes
    a["mainselector"]=mainselector
    SW_global.page_data_for_entire[str(len(SW_global.page_data_for_entire))]=a
    return


def reset_entire_page_data():
    global  letter_out_line_on_off,decision_dot_flag_pos,connect_dot_flag_pos,startdot_flag_pos,call_g,gl,gb,sl_t,sl_b,l,kern_value_array,delete_list,mainselector,guideline_axes
    global color_letter_features_on_off,start_arrow_button,decision_dot_button,decision_dot_button,color_letter_features_on_off
    SW_global.box_data.clear()
    SW_global.axes_data.clear()
    SW_global.startdot_on_off=0
    SW_global.stokearrow_on_off=0
    SW_global.connectdot_on_off=0
...
def select_which_page_to_redirect(axes_data=None):
    # print("I am in page redirect")
    ##### change here ######
    global guideline_axes,delete_list,kern_value_array,mainselector,connect_dot_flag_pos,startdot_flag_pos,\
    stoke_arrow_flag_pos,startdot_flag_pos,decision_dot_flag_pos,color_letter_features_on_off
    key=None
    if(axes_data!=None):
        # print("Ok check 1")
        # print("This is pagedat",SW_global.page_data_for_entire)
        for j in range(len(SW_global.page_data_for_entire)):
            # print("fig",SW_global.page_data_for_entire[str(j)]["fig"])
            if(SW_global.page_data_for_entire[str(j)]["fig"]==axes_data.get_figure()):
                key=j

    # print("This is key",key)
    if(key!=None):
        a=SW_global.page_data_for_entire[str(key)]
        # print("a is ",a)
        SW_global.global_figure=a["fig"]
        ##### changing here######
        # figAgg = FigureCanvasTkAgg(a["fig"], a["canvas"])
        # mplCanvas = figAgg.get_tk_widget()
        # cwid = canvas.create_window(0, 0, window=mplCanvas, anchor=Tkconstants.NW)
        # canvas.config(scrollregion=canvas.bbox(Tkconstants.ALL), width=(wd-24), height=550)
        SW_global.global_canvas= a["canvas"]
        ###### changing here #####
        SW_global.box_data=a["box_data"]
        # print("I am in wrong line",a["axes_data"])
        SW_global.axes_data=a["axes_data"]
        # guideline_axes[l].lines.clear()
        guideline_axes[l]=a["guideline_axes"]
        guideline_axes[l].lines=a["guideline_axes_lines"].copy()
        # guideline_axes[l].lines.extend(a["guideline_axes_lines"])
        SW_global.kern_list.clear()
        SW_global.kern_list.extend(a["SW_global.kern_list"])
        SW_global.cursor_data.clear()
        SW_global.cursor_data.extend(a["cursor_data"])
        SW_global.cursor_pos.clear()
        SW_global.cursor_pos.extend(a["cursor_pos"])
        kern_value_array.clear()
        kern_value_array.extend(a["kern_value_array"])
        delete_list.clear()
        delete_list.extend(a["delete_list"])
        compositedot_already_applied_array.clear()
        compositedot_already_applied_array.extend(a["compositedot_already_applied_array"])
        startdot_already_applied_array.clear()
        startdot_already_applied_array.extend(a["startdot_already_applied_array"])
        connectdot_already_applied_array.clear()
        connectdot_already_applied_array.extend(a["connectdot_already_applied_array"])
        decisiondot_already_applied_array.clear()
        decisiondot_already_applied_array.extend(a["decisiondot_already_applied_array"])
        SW_global.letters_already_written.clear()
        SW_global.letters_already_written.extend(a["SW_global.letters_already_written"])
        decision_dot_flag_pos=a["decision_dot_flag_pos"]
        connect_dot_flag_pos=a["connect_dot_flag_pos"]
        stoke_arrow_flag_pos=a["stoke_arrow_flag_pos"]
        startdot_flag_pos=a["startdot_flag_pos"]
        SW_global.recent_input_list.clear()
        SW_global.recent_input_list.extend(a["SW_global.recent_input_list"])
        SW_global.back_axes=a["SW_global.back_axes"]
        SW_global.global_figure.canvas.draw()
        guideline_axes[l].set_visible(True)
        SW_global.global_figure.canvas.draw()
        guideline_axes[l].set_visible(True)
        #### data transfering for button respected data #####
        color_letter_features_on_off=a["color_letter_features_on_off"]
        SW_global.connectdot_on_off=a["SW_global.connectdot_on_off"]
        SW_global.decisiondot_on_off=a["SW_global.decisiondot_on_off"]
        SW_global.startdot_on_off=a["SW_global.startdot_on_off"]
        SW_global.global_figure.canvas.draw()
    # print("I am in end of data switiching of pafge"*10)
        

    return
def update_entire_page_data(axes_data=None):
    # print("I am in update entire page")
    global guideline_axes,kern_value_array,delete_list,mainselector,color_letter_features_on_off
    key=None
    if(axes_data!=None):
        for j in range(len(SW_global.page_data_for_entire)):
            if(SW_global.page_data_for_entire[str(j)]["fig"]==axes_data.get_figure()):
                key=j
    if(key!=None):
        # print("I am in data upgrading part ")
        a=dict()
        a["fig"]=SW_global.global_figure
        a["canvas"]=canvas
        a["box_data"]=SW_global.box_data
        a["axes_data"]=SW_global.axes_data
        a["guideline_axes"]=guideline_axes[l]
        a["delete_list"]=delete_list.copy()
        a["guideline_axes_lines"]=(guideline_axes[l].lines).copy()
        a["kern_value_array"]=kern_value_array.copy()
        ...
        a["SW_global.back_axes"]=SW_global.back_axes
        a["mainselector"]=mainselector
        SW_global.page_data_for_entire[str(key)]=a
        SW_global.global_figure.canvas.draw()
        # print("I am in data upgradation end part"*10)
    return

def multipage_data_controller(axesdata=None):
    global guideline_axes,l
    # for j in range(len(guideline_axes[l].lines)):
    #     if j>=4:
    #         (guideline_axes[l].lines[j]).set_visible(False)
    # k=guideline_axes[l]
    # k1=[SW_global.axes_data[str(j)]["axis_data"] for j in range(len(SW_global.axes_data))]
    # print("I am in multipage data controller "*20)
    if(axesdata!=None):
        if(len(SW_global.page_data_for_entire)==0):
            # for j in range(len(guideline_axes[l].lines)):
            #     if j>=4:
            #         (guideline_axes[l].lines[j]).set_visible(False)
            save_data_for_entire_one_page(fig=SW_global.global_figure)
            # print("This is after multipage data controller")
            # print(guideline_axes[l])
            # print(SW_global.axes_data)
        else:
            for j in range(len(SW_global.page_data_for_entire)):
                if(SW_global.page_data_for_entire[str(j)]["fig"]==axesdata.get_figure()):
                    # print("I am in data upgradation part")
                    update_entire_page_data(axes_data=axesdata)
                    break
                elif j==len(SW_global.page_data_for_entire)-1:
                    save_data_for_entire_one_page(fig=SW_global.global_figure)
    return


def multipage_create(writingareaframe=None):
    global guideline_axes,mainselector
    guideline_axes[l].set_visible(True)
    

    # data_checking_for_need_to_create_or_update_back_page(axes_list=axes_list_for_box,delete_list1=delete_list,kern_value_array1=kern_value_array)
    writingareaframe12 = Frame(SW_Main_UI, width=(wd-5), height=500)
    writingareaframe13 = Frame(writingareaframe1, height=500, bd=1, relief=RAISED)
    start_pos_x=None
    start_pos_y=None
    if len(SW_global.axes_data)>0:
        start_pos_x=((SW_global.axes_data[str(0)]["axis_data"]).get_position()).x0
        start_pos_y=((SW_global.axes_data[str(0)]["axis_data"]).get_position()).y0
    else:
        if guideline_axes[l]!=None:
            start_pos_x=((guideline_axes[l]).get_position()).x0
            start_pos_y=((guideline_axes[l]).get_position()).y0
    multipage_data_controller(axesdata=guideline_axes[l])#SW_global.g_val)
    reset_entire_page_data()
    SW_global.back_axes=None
    SW_global.current_pos=0
    SW_global.current_axes=SW_global.g_val
    sub=Toplevel(writingareaframe12,bg="grey")
    fig=plt.figure()
    SW_global.global_figure=fig
    ...
    sub.overrideredirect(False)
    sub.lift()
    sub.wm_title("New Doc>>  "+sub.winfo_id().__str__())
    sub.geometry("1350x568+0+145")
    sub.wm_transient(writingareaframe12)
    sub.wm_attributes()
    
    # sub.grab_set()
    # sub.bind("<Button-1>",test1)
    ...
    # if restaurar_wn:
    #     addScrollingFigure(fig, writingareaframe13)
    # else:
    #     addScrollingFigure(fig, sub)
    canvas = Canvas(sub)
    canvas.grid(row=0, column=0, sticky=Tkconstants.NSEW)
    ...
    #     guideline_axes[l].set_position([start_pos_x,start_pos_y,guideline_axes[l].get_position().width,guideline_axes[l].get_position().height])
    fig.canvas.draw()
    fig.canvas.mpl_connect('key_press_event', press)
    fig.canvas.mpl_connect('button_press_event',onclick2)
    fig.canvas.mpl_connect('button_release_event',onrelease)
    ...
    base_array=[SW_global.g_val.lines[j] for j in range(4)]
    # list1=["A","S","X","Y"]
    # temp_kern_value,temp_kern_list,temp_delete,temp_recent,temp_cursor_data,temp_cursor_pos,letters_already_written2,temp_guideline_axes2=add_digit_in_axes(list_of_digit=list1,spec_axes=SW_global.g_val,baselines_objects_array=base_array)
    # newCreateGuideLine(1,None,None,None,None)
    SW_global.current_axes=SW_global.g_val
    SW_global.current_pos=0
    fig.canvas.draw()
...
def press(event):
    ...
    if(event.key=='ctrl+x'):
        ...
        fig.canvas.draw()
        SW_global.global_figure.canvas.draw()
        ### need to add font styling function ###
    if event.key == 'backspace':
        ...
        try:
            if(len(SW_global.axes_data)>0):
                ...
                # print("delete_list",delete_list)
                # print("kern_value_array",kern_value_array)
            ## need to add featurs for font styling 
        except Exception as e:
            print(e)
        SW_global.global_figure.canvas.draw()
            ...
            try:
                ...
                if(len(SW_global.axes_data)>0):
                    if SW_global.axes_data[str(0)]["axis_data"] in SW_global.text_flow_axes:
                        ...
                        ### main_add_controller_for_text_flow_features3(event_key=event.key)
                    else:
                        # print("check point 2")
                        main_add_controller(event.key)
                else:
...
def main_back_space_controller():
    ...
    global guideline_axes,kern_value_array,delete_list,compositedot_already_applied_array,startdot_already_applied_array,\
        decisiondot_already_applied_array,connectdot_already_applied_array,stoke_arrow_flag_pos,startdot_flag_pos,connect_dot_flag_pos,decision_dot_flag_pos
...
def delete_letter_from_end():
    global kern_x,startdot_already_applied_array,compositedot_already_applied_array,decisiondot_already_applied_array,\
    connectdot_already_applied_array,connect_dot_flag_pos,decision_dot_flag_pos,stoke_arrow_flag_pos,startdot_flag_pos
    ...
    try:
        ...
        # print("check point 1555")
        for i in range(srt_loop, end_loop):
            guideline_axes[l].lines[i].set_visible(False)
        # print("This is check point 233333")
        del SW_global.letters_already_written[len1-1]
        del SW_global.letters_already_written[len1-2]
        last_input_len = len(delete_list)
        last_glyph = delete_list[last_input_len - 1]
...
def combo_box_select(event):
    ...
    fig.canvas.draw()
    temp_delete_list=[]
    combo_item=combo_box.get()
    start_pos_x=None
    start_pos_y=None
    start_pos_y1=None
    if((int(combo_item)<=192) and (int(combo_item)>=8)):
        SW_global.current_font_size=int(combo_item)
        SW_global.gd_sc1=True
        # print("We can apply font size change ")
        for j in range(len(SW_global.axes_data)):
            temp_delete_list.extend(SW_global.axes_data[str(j)]["delete_list"])
            (SW_global.axes_data[str(j)]["axis_data"]).set_visible(False)
        temp_delete_list.extend(delete_list)
        guideline_axes[l].set_visible(False)
        SW_global.max_limit=((192-int(combo_item))*SW_global.unit_value_of_max_limit)+8000
        # print("This is SW_global.max_limit",SW_global.max_limit)
        SW_global.scl=((192-int(combo_item))*SW_global.unit_value_of_scl)+6
        # print("This is SW_global.scl",SW_global.scl)
        SW_global.current_width=-((192-int(combo_item))*SW_global.unit_value_of_width)+0.23
        if(len(SW_global.axes_data)>0):
            start_pos_x=((SW_global.axes_data[str(0)]["axis_data"]).get_position()).x0
            start_pos_y=((SW_global.axes_data[str(0)]["axis_data"]).get_position()).y0
            start_pos_y1=((guideline_axes[l]).get_position()).y1
...
#        newCreateGuideLine(1,None,None,None,None)
        ...
        if(len(temp)==0):
            newCreateGuideLine(1,None,None,None,None)
            if count1==0:
                start_pos_y=start_pos_y1-guideline_axes[l].get_position().height
            ...
            for j in range(len(temp)):
                ...
                for k in range(4):
                    base_array.append(guideline_axes[l].lines[k])
                temp_kern_value1,temp_kern_list1,temp_delete1,temp_recent1,temp_cursor_data1,temp_cursor_pos1,letters_already_written2,temp_guideline_axes2=add_digit_in_axes(list_of_digit=temp[j],spec_axes=guideline_axes[l],baselines_objects_array=base_array)
                if len1-1==count:
                    set_guideLine_variables(kern_value_array1=temp_kern_value1,delete_list1=temp_delete1,kern_list1=temp_kern_list1,recent1=temp_recent1,lines1=temp_guideline_axes2,cursor_pos1=temp_cursor_pos1,cursor_data1=temp_cursor_data1,letters_already_written=letters_already_written2)
                elif len1>count:
                    save_data_to_axes_dict(kern_value1=temp_kern_value1,delete_list1=temp_delete1,recent_input_list1=temp_recent1,cursor_pos1=temp_cursor_pos1,cursor_data1=temp_cursor_data1,axes_key_index=None,letters_already_written1=letters_already_written2,axesdata=guideline_axes[l],lines1=temp_guideline_axes2,kern_list1=temp_kern_list1)
                count=count+1
            SW_global.current_axes=guideline_axes[l]
            SW_global.current_pos=len(delete_list)-1
            if len(SW_global.cursor_data)>0:
        ...
        for j in range(len(SW_global.axes_data)):
            ...
            SW_global.axes_data[str(j)]["startdot_flag_pos"]=0
            (SW_global.axes_data[str(j)]["startdot_already_applied_array"]).clear()
            SW_global.axes_data[str(j)]["decision_dot_flag_pos"]
            (SW_global.axes_data[str(j)]["decisiondot_already_applied_array"]).clear()
        

        global connectdot_already_applied_array,connect_dot_flag_pos,stoke_arrow_flag_pos,compositedot_already_applied_array,\
            startdot_flag_pos,startdot_already_applied_array,decision_dot_flag_pos,decisiondot_already_applied_array
