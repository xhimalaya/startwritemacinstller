    ...
    try:
        ...
        if((pos1==10000)):
            if(cur_temp_start==1000):
                ...
                SW_global.single_click_axes_data=guideline_axes[l]
                fig.canvas.draw()
                return
            else:
                SW_global.single_click_data=(SW_global.axes_data[str(cur_temp_start)]["cursor_pos"])[-1]
                SW_global.single_click_axes_data=SW_global.axes_data[str(cur_temp_start)]["axis_data"]
            ...
            if((end_axes==guideline_axes[l]) and (start_axes==guideline_axes[l])):
                ...
                for i in range(len(SW_global.axes_data)):
                    if((SW_global.axes_data[str(i)]["axis_data"])==end_axes):
                        ...
                        SW_global.single_click_data=plot_data[0]
                        SW_global.single_click_axes_data=SW_global.axes_data[str(i)]["axis_data"]
                fig.canvas.draw()
    ...
    if SW_global.connectdot_on_off == 1:
        ...
        if connect_dot_flag_pos == 0:
            for i in range(delete_list_counter):
                ...
                #c1, c2 = manuscript_connect_dot.return_manuscript_fonts(glyph)
                 ##########################################himalaya########################################################
                skip_list=["A","E","Y","F","M","N","X","z","Z","x","K","[","]","W"]
...
def cut_addletter_for_multiple_guideline(rectangle_wigets_no=None,event_key=None):
    print("I am in cut_add guide line")
    pos1,temp=findPos1(axesdata=SW_global.click_axes,start_point=SW_global.click_x)
    print(pos1)
    if(SW_global.click_axes==guideline_axes[l]):
        print("check point 2")
        ...
        #if(pos1==-1):
            #pos1=0
        if(((pos1==10000) and (len(delete_list)==0)) or (pos1>=len(delete_list))):
            print("*"*60)
         #   print(pos1i)
          #  print(pos2i)
            print("*"*60)
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++ A-Z Input +++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    # A-Z manuscript
    try:
        cut_addletter_for_multiple_guideline(rectangle_wigets_no=None,event_key=event.key)
