...
def onrelease(event):
  #  print("This is on release &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
    if(guideline_axes[l]!=event.inaxes):
        for i in range(len(SW_global.axes_data)):
            if(event.inaxes==SW_global.axes_data[str(i)]["axis_data"]):
   #             print("I got that pointaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    ...
    pos2,cur_temp_end=findPos2(axesdata=start_axes,end_point=end_point)
    print("This is pos2")
    print(pos2)
    ...
    #Starting_loop_point1=SW_global.letters_already_written[Starting_loop_point]
    #ending_loop_point1=SW_global.letters_already_written[end_loop_point]
  ...
  #  print("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT")
    ...
    #   print("This is for check point colorwrite")
        ...
        if(SW_global.click_axes==SW_global.release_axes):
            if(SW_global.click_axes==guideline_axes[l]):
                ...
                count_for_select=0
               # for i in SW_global.g_val.lines:
                #    if((count_for_select>=(SW_global.letters_already_written[pos1*2])) and (count_for_select<=(SW_global.letters_already_written[(pos2*2)+1]))):
                 #       i.set_color("red")
                  #  count_for_select=count_for_select+1
                    #i.set_color("red")
                print("This is position1",pos1,"this is pos2",pos2)

                for i in range(SW_global.letters_already_written[pos1*2],SW_global.letters_already_written[(pos2*2)+1]):
                    print("check point echo2")
                    (guideline_axes[l].lines[i]).set_color("red")

                    #k=guideline_axes[l].lines[i]
                    #k.set_color('red')
                    #((guideline_axes[l].lines)[i]).set_color('red')
                    #fig.canvas.draw()
            else:
                for j in range(len(SW_global.axes_data)):
                    if(SW_global.axes_data[str(j)]["axis_data"]==SW_global.click_axes):
                        ...
                        #for k in range(len(SW_global.axes_data[str(j)]["letters_already_written"])):

                            # print("&"*60)
                            # print(len(SW_global.axes_data[str(j)]["lines"]))
                            # print(len(SW_global.axes_data[str(j)]["letters_already_written"]))
                            # print((SW_global.axes_data[str(j)]["letters_already_written"])[k])
                            # ((SW_global.axes_data[str(j)]["lines"])[(SW_global.axes_data[str(j)]["letters_already_written"])[k]]).set_color("red")
                        #break
        else:
            start_of_select=-999
            end_of_select=-999
            ...
            if(start_of_select==str(1000)):
                for h in range(len(SW_global.letters_already_written)):
                    if(h<=pos1*2):
                        ((guideline_axes[l].lines)[SW_global.letters_already_written[h]]).set_color("red")
                for i in range(len(SW_global.axes_data)):
                    if(i==end_of_select):
                        for h in range(len(SW_global.axes_data[str(i)]["letters_already_written"])):
                            if((pos2*2+1)>=h):
                                ((SW_global.axes_data[str(i)]["lines"])[(SW_global.axes_data[str(i)]["letters_already_written"])[h]]).set_color("red")
                    if(i>end_of_select):
                        count_for_select=0
                        for h in (SW_global.axes_data[str(i)]["lines"]):
                            if(count_for_select>3):
                                h.set_color("red")
                            count_for_select=count_for_select+1
                ...
                if(start_of_select>end_of_select):
                    for i in range(len(SW_global.axes_data)):
                        if(i==start_of_select):
                            for j in range(len(SW_global.axes_data[str(i)]["letters_already_written"])):
                                if(j<=pos1*2):
                                    if(j>3):
                                        ((SW_global.axes_data[str(i)]["lines"])[(SW_global.axes_data[str(i)]["letters_already_written"])[j]]).set_color("red")
                        if(i==end_of_select):
                            for j in range(len(SW_global.axes_data[str(i)]["letters_already_written"])):
                                if(j>=(pos2*2+1)):
                                    if(j>3):
                        ...
                        if((i>start_of_select) and (i<end_of_select)):
                            count_for_select=0
                            for j in SW_global.axes_data[str(i)]["lines"]:
                                if(count_for_select>3):
                                    j.set_color("red")
                else:
                    if(start_of_select<end_of_select):
                        for i in range(len(SW_global.axes_data)):
                            ...
                            if(i==end_of_select):
                                for j in range(len(SW_global.axes_data[str(i)]["letters_already_written"])):
                                    if(j<=(pos2*2+1)):
                                        if(j>3):
                                            ((SW_global.axis_data[str(i)]["lines"])[(SW_global.axes_data[str(i)]["letters_already_written"])[j]]).set_color("red")
                            if((i>start_of_select) and (i<end_of_select)):
                                count_for_select=0
                                for j in SW_global.axes_data[str(i)]["lines"]:
                                    if(count_for_select>3):
            ...
            #################### Condition where both of axes is not in current guideline axes(guideline_axes[l]) ##############
                        ...
                        #for h in range(len(SW_global.axes_data["lines"])):
                         #   if(count_for_select>3):
                          #      ((SW_global.axes_data["lines"])[h]).set_visible("red")
                #for i in range(len(guideline_axes[l].)
            #if((start_of_select>0) and (end_of_select>0)):
            # need to do it from there ###
    ...
    #else:
    #   print("This is 1 for colorwrite")
    ...
    if(SW_global.click_x==SW_global.release_x):
        ...
        #if()
        #print("This is single click data")
        #print(single_click_cursor_pos)
        #print(delete_list[pos1])
        ...
        if(SW_global.release_x>=SW_global.cursor_pos[-1]):
            ...
            plot_data=plt.plot(cursor_x, cursor_y, color='black', linewidth=0.6, dashes=(3, 4))
        ...
        #list(np.full((500),tempcu+300))

        #print("This is sw global cursor pos")
        #if((pos1+1)<=len(SW_global.cursor_pos)-1):
        #    print(pos1)
        #    print(pos2)
        #    print(SW_global.cursor_pos[pos1+1])
        #    print(delete_list[pos1])
        #else:
        #    print(pos1)
        #    print(SW_global.cursor_pos[-1])
        #    print(delete_list[pos1])

        #visible_item.set_visible(True)
        try:
            #print("This is temp_cursor temp data")
            #print(SW_global.temp_cursor_temp_data)
            item=SW_global.temp_cursor_temp_data[0]
            item.set_visible(False)
         #   print(SW_global.temp_cursor_temp_data)
        ...
        if(len(SW_global.cursor_data)>0):
            #v1=SW_global.cursor_data[start_ce]
            v2=SW_global.cursor_data[end_ce]
            #v1.set_visible(True)
            v2.set_color('black')
            v2.set_visible(True)
        tempcu=SW_global.cursor_pos[start_ce+1]
        #print("This is tempcu")
        #print(tempcu)
        cursor_y=list(np.linspace(-900,1500,500))
        if(tempcu==0):
        ...
        #if(tempcu==0):
        #   q=list(np.full((500),tempcu))
        #else:
        #   q=list(np.full((500),tempcu+300))
        cursor_x=q1 #list(np.full((500),tempcu+300))
        try:
        #   print("This is temp cursor 3")
            SW_global.temp_cursor_temp_data=plt.plot(cursor_x, cursor_y, color='black', linewidth=0.6, dashes=(3, 4))
        #   print(SW_global.temp_cursor_temp_data)
        except Exception as e:
        #   print(e)
            pass
    #print("This is end of mouse release")
    #print(pos1)
    #print(pos2)
        #print("This is pos100")
    fig.canvas.draw()
    return
