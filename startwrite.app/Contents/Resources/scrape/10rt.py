...
def set_text_for_text_flow():
    if(SW_global.text_flow_axes1==None):
        if(SW_global.text_flow_axes2==None):
            ...
            if(SW_global.text_flow_axes2==SW_global.current_axes):
                SW_global.text_flow_axes2=None
    else:
        if(SW_global.text_flow_axes2==None):
...
def combo_box_select(event):
    print(combo_box.get())
    #canvas.setFocusPolicy( tk.ClickFocus )
    #canvas.setFocus()
    #mainselector.focus_set()
    fig.canvas.mpl_connect('key_press_event', press)
    fig.canvas.mpl_connect('button_press_event',onclick2)
    fig.canvas.mpl_connect('button_release_event',onrelease)
    #event.wigets.master.focus_set()
    #main_frame.focus()
   # writingareaframe.focus()
    #SW_MainUI_.focus_set()
    fig.canvas.draw()
