...
def NewWindow(master):
    global all_windows
    fig=multipage_create(writingareaframe=writingareaframe)
...
def multipage_create(writingareaframe=None):
    ...
    sub=Toplevel(writingareaframe,bg="grey")
    fig=plt.figure()
...
################ Checking for wheather multiple guide line is needed or not #################
        ...
        if(len(str(event.key))==1):
            try:
                ...
                print("delete_list",delete_list)
                print("kern_value_array",kern_value_array)
                print("composit_dot",compositedot_already_applied_array)
                print("connect dot",connectdot_already_applied_array)
                print("start dot",startdot_already_applied_array)
                print("flag_connect",connect_dot_flag_pos)
                print("flag_start",startdot_flag_pos)
                print("flag_composite",stoke_arrow_flag_pos)
                print("button status start",SW_global.startdot_on_off)
                print("button status composite",SW_global.stokearrow_on_off)
                print("button_connect_status",SW_global.connectdot_on_off)
            except Exception as e:
                print(e)
                pass
