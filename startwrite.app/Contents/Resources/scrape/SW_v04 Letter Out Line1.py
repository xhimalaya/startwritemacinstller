...
def on_exit():
    ...
    if len(SW_global.recent_input_list) > 0:
        ...
        if MsgBox:
            ...
            pickle.dump(fig.gca(), open(filewin_saveas, "wb"))
            plt.close("all")
...
#-----------------------------------letter_shading_option_mw------------------------------------------#

v = tk.IntVar()  # letter_shading_option_mw variable
v.set(0)

# letter_shading_option_mw ----> Functions
def ShowChoice():
    print(v.get())
...
#--------------------------------------------------------------------------------------------------------#
#-----------------------------------letter_dot_density_option_mw------------------------------------------#

v1 = tk.IntVar()  # letter_dot_density_option_mw variable
v1.set(0)


# letter_dot_density_option_mw ----> Functions
def ShowChoice_1():
    print(v.get())
...
#--------------------------------------------------------------------------------------------------------#
#-----------------------------------stoke_arrows_option_mw------------------------------------------#

v = tk.IntVar()  # stoke_arrows_option_mw variable
v.set(0)

# stoke_arrows_option_mw ----> Functions
def ShowChoice():
    print(v.get())
...
#********************************background_menu_wtp close*****************************************#
#-----------------------------------color_letter_menu_wtp------------------------------------------#
...
#-------------------------------------------------------------------------------------------------------------------------------#
    ...
    Color_firstarea1 = tk.Button(Color_firstcolor_Frame_bgwn, bg=SW_global.first_letter_background_color, text='', width=40
                                ,command=setBgColor_bgwn)
...
#--------------------------------------------------------------------------------------------------------------------------------#
    ...
    background_button_ok = ttk.Button(backgroundEndokcnlFrame,
                                      text="OK",
                                      command=background_close)  # highlightbackground ='black'(change bottun change it work only in MacOSX)
    background_button_ok.pack(side=RIGHT, pady=2, anchor=E)

#--------------------------------------------------------------------------------------------------------------------------------#

#*************************************propertybar-frame-Option-windows End*******************************************************#
#************************************** WorkSheet Landscape & Portrait **********************************************************#

def landscape_gird():
    global f_w
    global f_h
    
    if SW_global.u_r == 1 and f_w == 6:
        f_w = 8
        f_h = 6
    if SW_global.w_r == 1 and f_w == 8:
        f_w = 9
        f_h = 8
...
def letter_shadding_25():
    ...
    global letter_shadding_on_off, alp
...
def letter_shadding_50():
    ...
    temp_recent_input_list = []
    temp_recent_input_list.extend(delete_list)
    # print("delete_list-delete_list 50", delete_list, len(delete_list))
    # print("temp_recent_input_list 50", temp_recent_input_list, len(temp_recent_input_list))
...
def letter_shadding_75():
    ...
    temp_recent_input_list = []
    temp_recent_input_list.extend(delete_list)
    # print("delete_list-delete_list 25", delete_list, len(delete_list))
    # print("temp_recent_input_list 25", temp_recent_input_list, len(temp_recent_input_list))
    ...
    # print("temp_recent_input_list", temp_recent_input_list)
...
def letter_shadding_100():
    ...
    # while len(delete_list) != 0:
    for it1 in range(len(delete_list)):
        len1 = len(SW_global.letters_already_written)
        len2 = len1 - 1
        srt_loop = SW_global.letters_already_written[len2 - 1]
        end_loop = SW_global.letters_already_written[len2]
...
def my_draw_shadding(c1, c2):
    global alp
    guideline_axes[l].plot(c1, c2, color='black', linewidth=0.7, dashes=(3, 4), alpha=alp)


def my_draw_shadding1(c1, c2):
    global alp
    guideline_axes[l].plot(c1, c2, color='black', linewidth=0.7, dashes=(3, 4), alpha=alp)
...
z1 = tk.StringVar() # zoom_wtp variable
z1.set("50%")  # zoom_wtp variable
    ...
    if SW_global.decisiondot_on_off == 1:
        ...
        if decision_dot_flag_pos == 0:
            for i in range(delete_list_counter):
                ...
                for iii in range(len(c1)):
                    for jjj in range(len(c1[iii])):
                        c1[iii][jjj] = c1[iii][jjj] + 110
    ...
    if SW_global.startdot_on_off == 1:
        ...
        if startdot_flag_pos == 0:
            for i in range(delete_list_counter):
                ...
                c1, c2 = manu_letter_start_dot.return_letter_outline_start_dot(glyph)
                c1, c2, draw_type = Kern_add_help.kern_add_operation(c1, c2, krn)
    ...
    if SW_global.stokearrow_on_off == 1:
        ...
        if stoke_arrow_flag_pos == 0:
            for i in range(delete_list_counter):
                ...
                c1, c2 = manu_letter_outline_arrow.return_outline_arrow_fonts(glyph)
                c1, c2, draw_type = Kern_add_help.kern_add_operation(c1, c2, krn)
...
def press(event):
    global init_letter_pos, final_letter_pos, startdot_flag_pos
    global stoke_arrow_flag_pos, connect_dot_flag_pos
    ...
    if event.key == 'enter':
        ...
        guideline_axes1_1.set_visible(True)

        if SW_global.gd_flag1:
            SW_global.lt_gd_1 = SW_global.new_left_axes1
            SW_global.btm_gd_1 = SW_global.new_bottom_axes1
            SW_global.wd_gd_1 = (SW_global.new_right_axes1-SW_global.lt_gd_1)
            SW_global.ht_gd_1 = tb_height
            guideline_axes1_1.set_position([SW_global.lt_gd_1, (SW_global.btm_gd_1-0.15), SW_global.wd_gd_1, SW_global.ht_gd_1])

            lft_gd = SW_global.new_left_axes1
            rht_gd = SW_global.new_right_axes1
            tp_gd = SW_global.new_top_axes1
            btm_gd = SW_global.new_bottom_axes1
            mainselector.extents = (lft_gd, rht_gd, (btm_gd-0.15), tp_gd)
            
        else:
            img = plt.imread('icons/apple.jpg')
            guideline_axes1_1.imshow(img, extent = [0.999,1,0.999,1])
            guideline_axes1_1.set_position([SW_global.lt_gd_1, (SW_global.btm_gd_1-0.15), SW_global.wd_gd_1, SW_global.ht_gd_1])
            mainselector.extents = (0.99,0.01, 0.68, 0.98)
            
        guideline_axes1_1.tick_params(axis='both', which='both', bottom='off', top='off', labelbottom='off', right='off', left='off', labelleft='off')
        default_guideline(guideline_axes1_1)
        
        for ln in ['top','right','left','bottom']:
            guideline_axes1_1.spines[ln].set_linewidth(0)
            fig.canvas.draw()

        mainselector.maxdist = 100
        SW_global.new_gd = 1
        fig.canvas.draw()
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++ A-Z +++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    # A-Z manuscript
    add_letter_with_outliner(event)
#     try:
#         length12 = len(SW_global.recent_input_list)
...
def add_letter_outliner_from_any_position(delete_list1=None,axesdata=None):
    ...
    if(len(SW_global.axes_data)>0):
        count_of_axes=0
        guideline_axes_lines2=SW_global.axes_data[str(0)]["axis_data"]
        for j in range(len(delete_list1)):
            ...
            if(kern_list2[0]>15500):
                if(count_of_axes<len(SW_global.axes_data)-1):
                    ...
                    SW_global.axes_data[str(count_of_axes)]["kern_list"]=[i for i in kern_list]
...
def my_draw_connect_dot(c1, c2):
    guideline_axes[l].plot(c1, c2, color=SW_global.connectingdot_color_var, linewidth=0.2)
...
def default_guideline(dynamic_axes):
    ...
    # =======================================================================================
        ...
        if SW_global.guidelines_toparea == 1:
            dynamic_axhtop = dynamic_axes.axhspan(790, 1460, facecolor=SW_global.guidelines_background_color_toparea, alpha=0.4)
            if SW_global.guidelines_background_color_toparea == '#ffffff':
                dynamic_axhtop = dynamic_axes.axhspan(790, 1460, facecolor=SW_global.guidelines_background_color_toparea, alpha=0.4)
            else:
        ...
        if SW_global.guidelines_middlearea == 1:
            dynamic_axhmiddle = dynamic_axes.axhspan(25, 730, facecolor=SW_global.guidelines_background_color_middlearea, alpha=0.4)
        ...
        if SW_global.guidelines_descenderarea == 1:
            dynamic_axhdescender = dynamic_axes.axhspan(-730, -20, facecolor=SW_global.guidelines_background_color_descenderarea, alpha=0.4)
...
def onclick(event):
    ...
    if event.inaxes == SW_global.g_val:
        ...
        mainselector.extents = (selector_dict[key_c][0], selector_dict[key_c][1], selector_dict[key_c][2], selector_dict[key_c][3])
        fig.canvas.draw()
...
portrait_button = tk.Button(maintoolbarframe, image=portrait_icon, command=portrait_gird)
...
landscape_button = tk.Button(maintoolbarframe, image=landscape_icon, command=landscape_gird)
...
x4 = IntVar()
x5_option = IntVar()
letter_dot_density_menu_button.menu.add_checkbutton(label="No Dots", variable=x0_no_dot)
letter_dot_density_menu_button.menu.add_checkbutton(label="25%", variable=x1)
letter_dot_density_menu_button.menu.add_checkbutton(label="50%", variable=x2)
letter_dot_density_menu_button.menu.add_checkbutton(label="75%%", variable=x3)
letter_dot_density_menu_button.menu.add_checkbutton(label="100%", variable=x4)
...
color_letter_button = tk.Button(propertybarframe, width=23, height=32, image=color_letter_icon)
