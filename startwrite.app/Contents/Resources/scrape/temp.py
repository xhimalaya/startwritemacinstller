...
def set_guideLine_variables(kern_value_array1=None,delete_list1=None,kern_list1=None,recent1=None,lines1=None,cursor_pos1=None,cursor_data1=None,letters_already_written=None):
    # print("I am in set guideline variables "*1000)
...
def onrelease(event):
    # print("I am from onrelease Please check "*100)
    ...
    print("This is for final testing ",(event.inaxes).get_figure()==(SW_global.g_val).get_figure())
    print("*"*200)
    ...
    if(guideline_axes[l].get_figure()==(event.inaxes).get_figure()):
        print("No need to change page"*10)
    else:
        print("need to change page"*10)
...
#            (SW_global.axes_data[str(0)]["axis_data"]).set_visible(False)
    ...
    cur_temp_start=-999
    cur_temp_end=-999
    print("This is start_point",start_point)
    print("This is start_axes",start_axes)
...
def NewWindow(master):
    ...
    # temp_kern_value,temp_kern_list,temp_delete,temp_recent,temp_cursor_data,temp_cursor_pos,letters_already_written2,temp_guideline_axes2=add_digit_in_axes(list_of_digit=["A","B","C","D"],spec_axes=SW_global.g_val,baselines_objects_array=base_array)
...
def default_guideline(dynamic_axes):
    ...
    global base_x, median_x, descender_x, ascender_x, base_y, median_y, descender_y, ascender_y
    global counter
    print("I am in default guideline")
    # Guideline Axes
...
def eraseStartDotFromMultipleGuideLine():
    # print("This is erase start dot from multiple guideline")
    for i in range(len(SW_global.axes_data)):
        # print("This is start_dot applied ",SW_global.axes_data[str(i)]["startdot_already_applied_array"])
        for j in range(len(SW_global.axes_data[str(i)]["startdot_already_applied_array"])-1):
            ...
            # print(en_pos)
            for k7 in range(st_pos,en_pos+1):
                ((SW_global.axes_data[str(i)]["lines"])[k7]).set_visible(False)
...
def check_sum_of_digit_advanced(array_value=None,key_axes=None):
    ...
    if((array_value!=None) and (key_axes!=None)):
        ...
        while(len(guideline_axes[l].lines)>=4):
            print("This is len",len(guideline_axes[l].lines))
            (guideline_axes[l].lines[len(guideline_axes[l].lines)-1]).set_visible(False)
            del guideline_axes[l].lines[len(guideline_axes[l].lines)-1]
        print("This is after",guideline_axes[l].lines)
        print("This is afer2",len(guideline_axes[l].lines))
        spec_axes=None
...
def newCreateGuideLine(n,a,b,c,d):
   ...
   #     SW_global.count_for_height=1
    ...
    if(SW_global.back_axes==None):
        ...
        SW_global.back_axes.spines["right"].set_linestyle('dashed')
        SW_global.back_axes.spines["right"].set_capstyle("butt")
        # SW_global.back_axes.plot([0, 0, 1,-1], [0, 0.5, 1,-10], 'o', clip_on=False, markersize=10,zorder=-5)
    #     x_number_list = [-9,-1,1, 4, 9, 16, 25]

    # # y axis value list.
    #     y_number_list = [-20,-10,1, 2, 3, 4, 5]
    #     SW_global.back_axes.scatter(x_number_list, y_number_list, s=10)
        # k=SW_global.back_axes.scatter([-100],[-100]) 
        # SW_global.back_axes.scatter([0],[0])
        # SW_global.back_axes.scatter([0],[0])
        # SW_global.back_axes.scatter([0],[0])
        # SW_global.back_axes.scatter([0],[0])
        # SW_global.back_axes.scatter([0],[0])
        # SW_global.back_axes.scatter([0],[0])
        # # k=SW_global.back_axes.plot([0],[100])
        # print("This is k",k)   
       # SW_global.back_axes.scatter([10],[100])
        #SW_global.back_axes.plot(,10)
        ...
        print("pos22",pos22.x0,pos22.y0,pos22.height,pos22.width)
        print("old_l",old_l,"old_b",old_b-height_axes)
        # calc_height=
    #     x_number_list = [-9,-1,1, 4, 9, 16, 25]
    ...
    #     SW_global.back_axes.scatter(x_number_list, y_number_list, s=10)
        ...
        print("pos33",SW_global.back_axes.get_position().x0,SW_global.back_axes.get_position().y0,SW_global.back_axes.get_position().height,SW_global.back_axes.get_position().width)
        print("*"*200)
    pos11=None
...
def save_data_for_entire_one_page(fig=None):
    ...
    global guideline_axes,kern_value_array,delete_list
    a=dict()
    if(fig!=None):
...
def select_which_page_to_redirect(axes_data=None):
    ...
    global guideline_axes,delete_list,kern_value_array,mainselector
    key=None
    if(axes_data!=None):
        print("Ok check 1")
    ...
    if(key!=None):
        ...
        SW_global.global_figure.canvas.draw()
        guideline_axes[l].set_visible(True)
        #### some thing which is wrong happen ####
        # for j in range(len(guideline_axes[l].lines)):
        #     if j>=4:
        #         guideline_axes[l].lines[j].set_visible(False)
...
def update_entire_page_data(axes_data=None):
    ...
    global guideline_axes,kern_value_array,delete_list,mainselector
    key=None
    if(axes_data!=None):
        for j in range(len(SW_global.page_data_for_entire)):
            if(SW_global.page_data_for_entire[str(j)]["fig"]==axes_data.get_figure()):
        ...
        if(len(SW_global.page_data_for_entire)==0):
            ...
            for j in range(len(SW_global.page_data_for_entire)):
                if(SW_global.page_data_for_entire[str(j)]["fig"]==axesdata.get_figure()):
                    ...
                    save_data_for_entire_one_page(fig=SW_global.global_figure)
    print("This is after controller")
    print("This is guideline_axes[l]",guideline_axes[l])
    # for j in range(len(SW_global.axes_data)):
    #     print("This is axes_data",SW_global.axes_data[str(j)]["axis_data"])
    # if k==guideline_axes[l]:
    #     print("This is wrong")
    # else:
...
def multipage_create(writingareaframe=None):
    print("I am multipage data controller")
    # axes_list_for_box=[(SW_global.axes_data[str(j)]["axis_data"]) for j in range(len(SW_global.axes_data))]
    # axes_list_for_box.append(SW_global.g_val)
...
def press(event):
    ...
    print("This is kern_list",SW_global.kern_list)
    print("This is guideline_axes[l].lines",guideline_axes[l].lines)
    # guideline_axes[l].set_position([0,0,1,1])
    print("*"*200)
    print("This is the best check I can ",SW_global.global_figure==guideline_axes[l].get_figure())
    # k_fig=fig
    # if(SW_global.global_figure!=None):
    #     print("I am in figure changing part ")
    #     k_fig=SW_global.global_figure
    #     newCreateGuideLine(1,None,None,None,None)
    #     fig.canvas.draw()
    # fig.canvas.draw()
    ...
    if(event.key=='ctrl+v'):
        ...
        pos4=-1
        paste_on_axes()
        ### need to add funtion for dot styling function
        # print(delete_list)
...
#        newCreateGuideLine(1,None,None,None,None)
        ...
        #         for k in range(4):
        #             base_array.append(guideline_axes[l].lines[k])
        #         temp_kern_value1,temp_kern_list1,temp_delete1,temp_recent1,temp_cursor_data1,temp_cursor_pos1,letters_already_written3,temp_guideline_axes1=add_digit_in_axes(list_of_digit=temp[j],spec_axes=guideline_axes[l],baselines_objects_array=base_array)
        #         if(len1==count):
        #             print()
        #             set_guideLine_variables(kern_value_array1=temp_kern_value1,delete_list1=temp_delete1,kern_list1=SW_global.kern_list1,recent1=temp_recent1,lines1=temp_guideline_axes1,cursor_pos1=temp_cursor_pos1,cursor_data1=temp_cursor_data1,letters_already_written=letters_already_written3)
        #         elif(len1>count):
        #             save_data_to_axes_dict(kern_value1=temp_kern_value1,delete_list1=temp_delete1,recent_input_list1=temp_recent1,cursor_pos1=temp_cursor_pos1,cursor_data1=temp_cursor_data1,axes_key_index=None,letters_already_written1=letters_already_written2,axesdata=guideline_axes[l],lines1=temp_guideline_axes2,kern_list1=temp_kern_list)
        #         count=count+1
        fig.canvas.draw()
        SW_global.global_figure.canvas.draw()
        ...
        print("We can't apply font size change")
