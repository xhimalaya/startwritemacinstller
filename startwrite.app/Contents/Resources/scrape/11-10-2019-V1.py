...
def add_digit_in_axes(list_of_digit=None,spec_axes=None,baselines_objects_array=None):
    ...
    if((list_of_digit!=None) and(spec_axes!=None)):
        kern_list2=[0]
        kern_value_array2=[0]
        delete_list2=[]
        cursor_pos2=[0]
        ...
        if(SW_global.current_axes==guideline_axes[l]):
            if((SW_global.kern_list[0]>SW_global.max_limit) and((SW_global.current_pos==None) or(SW_global.current_pos>=SW_global.cursor_pos[len(SW_global.cursor_pos)-1]))):
                ...
                if(len(temp10)>1):
                    ...
                    print("Temp10",temp10)
                    if(len(SW_global.axes_data)>0):
                        text_flow_main(list1=temp10[1],zero_axes=SW_global.axes_data[str(0)]["axis_data"])
                    else:
                        text_flow_main(list1=temp10[1],zero_axes=guideline_axes[l])
                    ## Testing print
                    fig.canvas.draw()
                    print("This is SW_global.recent_input_list",SW_global.recent_input_list)
...
def change_controller(key=None,axesdata=None):
    ...
    if(guideline_axes[l]==axesdata):
        ...
        if(len(SW_global.axes_data)>0):
            ...
            #SW_global.current_axes=axesdata
            axes_list_for_box=[]
            for j in range(len(SW_global.axes_data)):
                axes_list_for_box.append(SW_global.axes_data[str(j)]["axis_data"])
            axes_list_for_box.append(guideline_axes[l])
                    ...
                    if(check_axes in SW_global.box_data[str(j)]["axes_list"]):
                        ...
                        if(len(SW_global.axes_data)>0):
                            ...
                            if(len(temp10)>1):
                                ...
                                while(len(guide_axes[l].lines)>4):
                                    del guideline_axes[l].lines[len(guideline_axes[l].lines)-1]
...
def letter_manupulation_after_apply_text_flow(need_array=None,next_axes1=None,key_for_axes_no=None,axesdata=None):
    ...
    if((next_axes1!=None) and (key_for_axes_no!=None) and (axesdata!=None)):
        ...
        if(" " in need_array):
            for k0 in need_array:
                ...
                if(k0==" "):
                    ...
                    for k1 in range(count2,count1+1):
                        temp1.append(need_array[k1])
                    temp.append(temp1.copy())
                    temp1.clear()
                    count1=count2+1
            ...
            for j in range(len(SW_global.axes_data)):
                ...
                if(SW_global.axes_data[str(j)]["axis_data"]==current_axes1):
                    ...
                    for k in range(len(SW_global.axes_data[str(j)]["cursor_pos"])):
                       #  print("loop4")
                       #  print(k)
                       #  print((SW_global.axes_data[str(j)]["cursor_pos"])[6])
                       #  print("end of check")
                       # # print((len(SW_global.axes_data[str(j)]["cursor_pos"])-1>=k+1) and ((SW_global.axes_data[str(j)]["cursor_pos"])[k]<=SW_global.current_pos) and(SW_global.cursor_pos[k+1]>=SW_global.current_pos))
                       #  print("This is check   111")
                       #  print(len(SW_global.axes_data[str(j)]["cursor_pos"]))
                       #  print(SW_global.axes_data[str(j)]["cursor_pos"][k])
                       #  print((len(SW_global.axes_data[str(j)]["cursor_pos"])-1>=k+1))
                       #  print((SW_global.axes_data[str(j)]["cursor_pos"])[k]<=SW_global.current_pos)
                       # print(SW_global.cursor_pos[k+1])
...
def new_add_with_backspace(event_key=None):
...
def newCreateGuideLine(n,a,b,c,d):
   ...
   #     SW_global.count_for_height=1
    ...
    if(SW_global.back_axes==None):
        SW_global.back_axes=plt.axes([old_l, old_b-height_axes, 0.98, 0.156])
...
def set_text_for_text_flow():
    ...
    print("SW_global.text_flow_pos",SW_global.text_flow_pos)
        ...
        # if(SW_global.text_flow_axes1!=None):
        #     print("Text_flow_axes1 pos",SW_global.text_flow_axes1.get_position().bounds)
        # if(SW_global.text_flow_axes2!=None):
        #     print("Text_flow_axes1 pos",SW_global.text_flow_axes2.get_position().bounds)
        # if(SW_global.current_axes in SW_global.text_flow_axes):
        #     print("need to do some speacial operation")
        #     for j in range(len(SW_global.text_flow_axes)):
        #         if(SW_global.current_axes==SW_global.text_flow_axes[j]):
        #             del SW_global.text_flow_axes[j]
        #             break
        # else:
        #     SW_global.text_flow_axes.append(SW_global.current_axes)  
    return
    ...
    # print("I am working ")
    # print(SW_global.entire_delete_list_for_one_page)
