...
def backspaceOperation2():
    ...
    if((SW_global.current_axes==None) and (SW_global.current_pos==None)):
        print("check point 2")
        if(SW_global.que_flag!=None):
            if((len(delete_list)==0) and (SW_global.once==None)):
                ...
                SW_global.current_pos_in_number=len(SW_global.axes_data[str(len(SW_global.axes_data)-1)]["delete_list"])
                SW_global.once=10
